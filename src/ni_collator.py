...
@dataclass
class DataCollatorForNIInstruction:
    ...
    def __call__(self, batch, return_tensors=None):
        ...
        for instance in batch:
            ...
            # source = task_name + definition + "".join(pos_examples) + "".join(neg_examples) + task_input
            source = "Now write a task definition for the following examples \n" + task_name + "".join(pos_examples) + "".join(neg_examples) + task_input
            ...
            if len(tokenized_instruction) <= self.max_target_length:
                instructions.append(definition)
            else:
                instructions.append(self.tokenizer.decode(tokenized_instruction[:self.max_target_length], skip_special_tokens=True))
